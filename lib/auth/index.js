...
const internals = {
  encrypt: Promise.promisify(Crypto.pbkdf2)
};
 ...
 */
internals.verifyPassword = (password, hash, salt) => {

  return internals.encrypt(password, salt, 10000, 512, 'sha1')
    .then((dk) => {
 ...
 *
 * @param {string} password
 * @returns {Promise.<{hash, salt}>}
 */
internals.hashPassword = (password) => {

  if (!internals.isPasswordValid(password)) {
    return Promise.reject(Boom.badRequest('Password does not meet minimum requirements'));
  }
  ...
  return internals.encrypt(password, salt, 10000, 512, 'sha1')
    .then((dk) => {
      const key = new Buffer(dk, 'binary').toString('hex');
      return Promise.resolve({'hash': key, 'salt': salt});
    });
};
